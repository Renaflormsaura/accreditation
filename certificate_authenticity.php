...
$table = "certificate_authenticity";
    ...
    if (!$error) {
        if (!empty($title) && !empty($description)) {
            if ($id > 0) {
                ...
                $message = $success ? "Certificate entry updated successfully." : "Failed to update.";
            } else {
                if ($image_uploaded) {
                    $stmt = $pdo->prepare("INSERT INTO $table (title, description, image) VALUES (?, ?, ?)");
                    $success = $stmt->execute([$title, $description, $new_img_name]);
                    $message = $success ? "New certificate entry added." : "Failed to add new.";
                } else {
...
if (isset($_GET['delete'])) {
    ...
    header("Location: certificate_authenticity.php");
    exit();
}
...
<title>Certificate of Authenticity - Admin - PUP Unisan Accreditation</title>
<style>
...
<h1>Manage Certificate of Authenticity</h1>
...
<form method="POST" enctype="multipart/form-data" aria-label="<?php echo $edit ? 'Edit Certificate Entry' : 'Add Certificate Entry'; ?>">
    ...
    <button type="submit"><?php echo $edit ? 'Update Entry' : 'Add Entry'; ?></button>
    <?php if ($edit): ?>
        <a href="certificate_authenticity.php" style="margin-left:16px; background:#800000; padding:14px 22px; border-radius:12px; color:#fff; font-weight:600; text-decoration:none;">Cancel</a>
    <?php endif; ?>
</form>

<section class="entries" aria-label="List of Certificate Entries">
<?php if (!$entries): ?>
    <p>No certificate entries found.</p>
<?php else: ?>
    <?php foreach($entries as $entry): ?>
        <article class="entry">
            ...
            <div class="entry-actions">
                <a href="certificate_authenticity.php?edit=<?php echo (int)$entry['id']; ?>" aria-label="Edit <?php echo htmlspecialchars($entry['title']); ?>">Edit</a>
                <form method="GET" onsubmit="return confirm('Delete this entry?');" aria-label="Delete <?php echo htmlspecialchars($entry['title']); ?>" >

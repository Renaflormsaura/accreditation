    ...
    if (!$error) {
        if (!empty($title) && !empty($description)) {
            if ($id > 0) {
                ...
                $message = $success ? "Area survey updated successfully." : "Failed to update.";
            } else {
                if ($image_uploaded) {
                    $stmt = $pdo->prepare("INSERT INTO $table (title, description, image) VALUES (?, ?, ?)");
                    $success = $stmt->execute([$title, $description, $new_img_name]);
                    $message = $success ? "New area survey added." : "Failed to add new.";
                } else {
...
if (isset($_GET['delete'])) {
    ...
    header("Location: area_surveys.php");
    exit();
}
...
<title>Area Under Surveys - Admin - PUP Unisan Accreditation</title>
<style>
...
<h1>Manage Area Under Surveys</h1>
...
<form method="POST" enctype="multipart/form-data" aria-label="<?php echo $edit ? 'Edit Area Survey' : 'Add Area Survey'; ?>">
    ...
    <?php if ($edit): ?>
        <a href="area_surveys.php" style="margin-left:16px; background:#800000; padding:14px 22px; border-radius:12px; color:#fff; font-weight:600; text-decoration:none;">Cancel</a>
    <?php endif; ?>
</form>

<section class="entries" aria-label="List of Area Surveys">
<?php if (!$entries): ?>
    <p>No area surveys found.</p>
<?php else: ?>
    <?php foreach($entries as $entry): ?>
        <article class="entry">
            ...
            <div class="entry-actions">
                <a href="area_surveys.php?edit=<?php echo (int)$entry['id']; ?>" aria-label="Edit <?php echo htmlspecialchars($entry['title']); ?>">Edit</a>

    ...
    if (!$error) {
        if (!empty($title) && !empty($description)) {
            if ($id > 0) {
                ...
                $message = $success ? "About PUP Unisan entry updated successfully." : "Failed to update.";
            } else {
                if ($image_uploaded) {
                    $stmt = $pdo->prepare("INSERT INTO $table (title, description, image) VALUES (?, ?, ?)");
                    $success = $stmt->execute([$title, $description, $new_img_name]);
                    $message = $success ? "New About PUP Unisan entry added." : "Failed to add new.";
                } else {
...
if (isset($_GET['delete'])) {
    ...
    header("Location: about_pup_unisan.php");
    exit();
}
...
<title>About PUP Unisan - Admin - PUP Unisan Accreditation</title>
<style>
...
<form method="POST" enctype="multipart/form-data" aria-label="<?php echo $edit ? 'Edit About Entry' : 'Add About Entry'; ?>">
    ...
    <?php if ($edit): ?>
        <a href="about_pup_unisan.php" style="margin-left:16px; background:#800000; padding:14px 22px; border-radius:12px; color:#fff; font-weight:600; text-decoration:none;">Cancel</a>
    <?php endif; ?>
</form>

<section class="entries" aria-label="List of About Entries">
<?php if (!$entries): ?>
    ...
    <?php foreach($entries as $entry): ?>
        <article class="entry">
            ...
            <div class="entry-actions">
                <a href="about_pup_unisan.php?edit=<?php echo (int)$entry['id']; ?>" aria-label="Edit <?php echo htmlspecialchars($entry['title']); ?>">Edit</a>
